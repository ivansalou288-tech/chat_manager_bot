...
@dp.message_handler(Text(startswith=["муты"], ignore_case=True), content_types=ContentType.TEXT) #Функция размута
async def mutes_check(message):
    ...
    for i in range(mutes_count):
        print(users_ids[i])
        name_user = await bot.get_chat_member(chat_id=message.chat.id, user_id=int(users_ids[i]))
    ...
    await message.answer(f'⚪️ <b>Список пользователей, которым запрещено писать:</b>\n\n{itog_text}', parse_mode=ParseMode.HTML)

@dp.message_handler(Text(startswith='мут', ignore_case=True), content_types=ContentType.TEXT) #Мут
async def mute(message):

    global klan_chat_id, is_auto_unmute
    ...
    try:
        ...
        user_id = cursor.execute(f"SELECT tg_id FROM [{-(message.chat.id)}] WHERE username=?", (username,)).fetchall()[0][0]
        name_user = cursor.execute(f"SELECT nik FROM [{-(message.chat.id)}] WHERE username=?", (username,)).fetchall()[0][0]
    except IndexError:
    ...
    if await is_more_moder(user_id, moder_id, message.chat.id) == False:
        ...
        return
    # ------------------------------------------------------------------------------------------------
    print(muteint, mutetype, comments)
    a = await mute_user(user_id, message.chat.id, muteint, mutetype, message, comments)
    if a == True:
        await message.reply(f'🔇<b>Нарушитель:</b> <a href="tg://user?id={user_id}">{name_user}</a> лишается права слова\n⏰<b>Срок наказания:</b> {muteint} {mutetype}\n<b>👿Наказал его:</b> {moder_link}\n💬<b>Нарушение: {comments}</b>', parse_mode='html')
...
@dp.message_handler(Text(startswith=['анмут', "размут"], ignore_case=True), content_types=ContentType.TEXT) #Функция размута
...
@dp.message_handler(Text(startswith='бан', ignore_case=True), content_types=ContentType.TEXT)#Функция бана
async def ban(message):
    ...
    if await ban_user(user_id, message.chat.id, user_men, moder_men, comments, message_id, message) == True:
        ...
        await message.reply(f'<b>❗️Внимание❗️</b>\n🔴Злостный нарушитель <a href="tg://user?id={user_id}">{name_user}</a> получает бан и покидает нас\n👮‍♂️Выгнал его: {moder_link}\n💬Выгнали его за: {comments}',parse_mode='html')

@dp.message_handler(Text(startswith='причина бана', ignore_case=True), content_types=ContentType.TEXT)#Функция бана
async def prich_ban(message):
    ...
    await message.reply(f'🚨Нарушитель {user_men} был забанен навсегда\n💬Причина: {comments}\n👮‍♂️Забанил: {moder_men}\n⏰Когда: {date}\n📝Айди в пабге: {pubg_id}\n📨<a href="{message_link}">Прейти к сообщению</a>', parse_mode='html')

@dp.message_handler(Text(startswith='разбан', ignore_case=True), content_types=ContentType.TEXT)#Функция разбана
async def unban(message):
    ...
    await message.reply(f' ✅ Пользователь <a href="tg://user?id={user_id}">{name_user}</a> разбанен\n👮‍♂️Помиловал его: {moder_link}\n\n💬<a href="tg://user?id={user_id}">{name_user}</a>, мы ждем твоего возвращения!',parse_mode='html')

@dp.message_handler(Text(startswith='кик', ignore_case=True), content_types=ContentType.TEXT)#Функция кика
async def kick(message):
    ...
    if await kick_user(user_id, message.chat.id) == True:
        await message.reply(f'❎ <a href="tg://user?id={user_id}">{name_user}</a> покидает нас с возможностью возвращения\n👮‍♂️Выгнал его: {moder_link}\n💬Причина изгнания: {comments}',parse_mode='html')


@dp.message_handler(commands=["id"], content_types=ContentType.TEXT)#Функция узнавания айди чата
async def id_chat(message):
    await message.reply(f'айди чата "<code>{message.chat.id}</code>"', parse_mode='html')

@dp.message_handler(Text(startswith="пинг", ignore_case=True), content_types=ContentType.TEXT)#проверка работоспособности бота
...
@dp.message_handler(Text(startswith="бот", ignore_case=True), content_types=ContentType.TEXT)#проверка работоспособности бота
...
@dp.message_handler(Text(startswith='Постинг', ignore_case=True), content_types=ContentType.TEXT) #Постинг напоминание в группу "замы"
...
@dp.message_handler(Text(startswith=['созыв', 'созвать', 'общий сбор'], ignore_case=True), content_types=ContentType.TEXT)#Общий сбор
async def all_sbor(message):
    ...
    if await is_successful_moder(moder_id, message.chat.id, 'all') == False:
        ...
        return
    connection = sqlite3.connect('Base_bot.db')
    cursor = connection.cursor()
    try:
...
@dp.message_handler(Text(startswith=["преды", 'варны'], ignore_case=True), content_types=ContentType.TEXT)#Просмотр варнов своих и другово пользователя
async def warns_check(message: types.Message):
    ...
    text = await warn_check_sdk(tg_id, message.chat.id, name_user)
    await message.reply(text, parse_mode='html')

@dp.message_handler(Text(startswith=['пред', 'варн'], ignore_case=True), content_types=ContentType.TEXT)#Выдача преда
async def warnUser(message: types.Message):
    ...
    await give_warn(message=message, comments=comments, warn_count_new=warn_count_new, user_id=user_id, is_first=is_first)
    await message.reply(f'🛑 Нарушитель <a href="tg://user?id={user_id}">{name_user}</a> нарушил правила и получает предупреждение <b>({warn_count_new}/3)</b>\n<b>👮‍♂️Поймал его:</b> {moder_link}\n<b>💬Нарушение:</b> {comments}\n\n<a href="tg://user?id={user_id}">{name_user}</a>, больше так не делай, соблюдай правила!', parse_mode='html')
    if warn_count_new == 3:
...
@dp.message_handler(Text(startswith=['снять пред', 'снять варн'], ignore_case=True), content_types=ContentType.TEXT) #Снятие преда
async def snat_warnUser(message: types.Message):
    ...
    try:
        ...
        if not message.reply_to_message:
            await message.reply('📝Невозможно найти информацию о пользователе\n\n💬Введите корректный юзернейм(<code>@</code><i>юзер</i>) или ответь на сообщение нужного пользователя', parse_mode='html')
            return
        user_id = message.reply_to_message.from_user.id
        name_user = message.reply_to_message.from_user.first_name
    if message.chat.id == message.from_user.id:
    ...
    if warns_count == 0:
        await message.reply(f'❕Предупреждения <a href="tg://user?id={user_id}">{name_user}</a> отсутствуют', parse_mode='html')
        return
    ...
    if warn_count_dell > warns_count:
        await message.reply('❕Предупреждение с таким номером отсутвует!\n\n💬<i>Предупреждения пользователя можно узнать по команде</i>«<code>преды @</code><i>юзер</i>»', parse_mode='html')
        return
    await snat_warn(user_id=user_id, number_warn=warn_count_dell, warn_count_new=warn_count_new, message=message)
    await message.reply(f'✅<a href="tg://user?id={user_id}">{name_user}</a>, с тебя сняли одно предупреждение\n👮‍♂️Добрый модер: {moder_link}\n💬Количество твоих предупреждений: {warn_count_new} из 3\n\n<i>Свои предупреждения ты можешь посмотреть по команде</i> «<code>преды</code>»', parse_mode='html')

    connection.commit()
    connection.close()


@dp.message_handler(Text(startswith=['снятые преды', 'снятые варны'], ignore_case=True))  # Снятие преда
async def snatie_warnUser(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,f'🗓<b>Снятые предупреждения этого пользователя(страниц: {page_c}):</b>\n\n{itog[page]}', parse_mode='html',
                        reply_markup=keyboard)
    await message.answer('🗓Список снятых предупреждений пользователя отправлен в <a href="https://t.me/for_klan_tests_bot">лс</a>', parse_mode=ParseMode.HTML, disable_web_page_preview=True)
...
async def successful_recom(call: types.CallbackQuery):
    ...
    try:
        ...
        if page < 0:
            ...
            return
        await call.message.edit_text(f'🗓<b>Снятые предупреждения этого пользователя(страниц: {page_c}):</b>\n\n{itog[page]}', parse_mode='html',
                        reply_markup=keyboard)
        # print(page)
    except IndexError:
        page += 1
...
@dp.message_handler(Text(startswith="повысить", ignore_case=True), content_types=ContentType.TEXT)#повысить пользователя
async def rang_up(message: types.Message):
    ...
    try:
        ...
        name_user = message.reply_to_message.from_user.first_name
    try:
        rang_moder = cursor.execute(f"SELECT rang FROM [{-(message.chat.id)}] WHERE tg_id=?", (moder_id,)).fetchall()[0][0]
    except IndexError:
        await message.reply(
    ...
    #Повышаем
    try:
        first_rang_user = cursor.execute(f"SELECT rang FROM [{-(message.chat.id)}] WHERE tg_id=?", (user_id,)).fetchall()[0][0]
    except IndexError:
    ...
    rangs_name = ('Обычный участник', 'Младший Модератор', 'Модератор', 'Старший Модератор', 'Заместитель', 'Менеджер',
                  'Владелец')
    await message.reply(
        f'✅Ранг <a href="tg://user?id={user_id}">{name_user}</a> назначен(а): {rangs_name[new]}[{new}]', parse_mode="html")
    connection.commit()
    connection.close()


@dp.message_handler(Text(startswith=["понизить","занизить"], ignore_case=True), content_types=ContentType.TEXT)#понизить пользователя
...
@dp.message_handler(Text(startswith=["снять","разжаловать"], ignore_case=True), content_types=ContentType.TEXT)#снять пользователя с поста модератора
...
@dp.message_handler(Text(startswith="описание", ignore_case=True), content_types=ContentType.TEXT)#просмтр описания пользователя
async def about_user(message: types.Message):
    try:
        ...
        await message.reply(text=text,reply_markup=keyboard, parse_mode="html")
         ...
         await message.reply(f'Описание <a href="tg://user?id={message.reply_to_message.from_user.id}">Пользователя</a> не заполнено', parse_mode="html")

@dp.message_handler(Text(startswith="-чат", ignore_case=True), content_types=ContentType.TEXT)#закрыть чат
async def minus_chat(message):
    ...
    keyboard.add(*buttons)
    await message.reply(f'🤐 <b>Чат закрыт для общения</b>\nТеперь писать в чат могут только администраторы\n\n💬<i> Чат можно открыть по команде «</i><code>+чат</code><i>»</i> или нажав на кнопку снизу', reply_markup=keyboard, parse_mode="HTML")


@dp.message_handler(Text(startswith="+чат", ignore_case=True), content_types=ContentType.TEXT)#открыть чат
async def open_chat(message):
    ...
    await message.reply(f'✅ Чат открыт для общения\n<i>Теперь у всех есть разрешение на отправку сообщений</i>', parse_mode="HTML")
...
async def open_chat_button(call):
    ...
    if await is_successful_moder(moder_id, call.message.chat.id, 'close_chat') == False:
        await bot.answer_callback_query(call.id, text='📝Ранг модератора не достаточен для использования этой команды', show_alert=True)
        return
    elif await is_successful_moder(moder_id, call.message.chat.id, 'close_chat') == 'Need reg':
        await bot.answer_callback_query(call.id, text='📝Для использования бота нужно зарегистрироваться\n\n💬<i>Для регистрации напиши @zzoobank, он все объяснит</i>', show_alert=True)
        return
    await bot.set_chat_permissions(call.message.chat.id, ChatPermissions(can_send_messages=True, can_send_media_messages=True,
                                                               can_send_photos=True, can_send_videos=True,
                                                               can_send_audios=True, can_send_documents=True,
                                                               can_send_other_messages=True,
                                                               can_send_video_notes=True, can_send_voice_notes=True,
                                                               can_pin_messages=True,
                                                               can_add_web_page_previews=True, can_send_polls=True))
    await bot.send_message(call.message.chat.id, '✅ Чат открыт для общения\n<i>Теперь у всех есть разрешение на отправку сообщений</i>', parse_mode="HTML")


@dp.message_handler(Text(startswith='кто админ', ignore_case=True), content_types=ContentType.TEXT) #просмотр админов чата
async def kto_admin(message):
    ...
    for user in users_6rang:
        rang_6.append(f'<a href="tg://user?id={user[0]}">{cursor.execute(f"SELECT nik FROM [{-(message.chat.id)}] WHERE tg_id = ?", (user[0],)).fetchall()[0][0]}</a>')
    ...
    for user in users_5rang:
        rang_5.append(
            f'<a href="tg://user?id={user[0]}">{cursor.execute(f"SELECT nik FROM [{-(message.chat.id)}] WHERE tg_id = ?", (user[0],)).fetchall()[0][0]}</a>')
...
@dp.message_handler(Text(startswith='ник', ignore_case=True), content_types=ContentType.TEXT)# просмотр ника
...
@dp.message_handler(Text(startswith='+ник', ignore_case=True), content_types=ContentType.TEXT) #изменение ника
async def plus_nik(message):
    ...
    await message.reply(f'Ник пользователя установлен на: {comments}')


@dp.message_handler(Text(startswith='дк', ignore_case=True), content_types=ContentType.TEXT) #изменение мигимального рангда команд
async def dk(message):
    connection = sqlite3.connect('Base_bot.db', check_same_thread=False)
    cursor = connection.cursor()
    if message.chat.id == klan_chat_id:
        rang_up_dk = int(cursor.execute("SELECT dk FROM klan WHERE comand=?", ("dk",)).fetchall()[0][
                             0])  # Ранг с которого можно повышать
    else:
        rang_up_dk = int(cursor.execute("SELECT dk FROM sostav WHERE comand=?", ("dk",)).fetchall()[0][
                             0])
    rang_moder = \
    cursor.execute(f"SELECT rang FROM [{-(message.chat.id)}] WHERE tg_id=?", (message.from_user.id,)).fetchall()[0][0]
    ...
    if rang_dk > 0 and rang_dk <=6:
        await message.reply(f"{num[rang_dk]} Команда «{command}» теперь доступна с ранга модератора {rangs_name[rang_dk]} ({rang_dk})")
    if rang_dk == 0:
...
async def recom_check(message):
    ...
    if text == '':
        await message.reply(f'📝Рекомендации <a href="tg://user?id={tg_id}">{name_user}</a> отсутвуют', parse_mode='html')
        return
    await message.reply(f'{text}', parse_mode='html')


@dp.message_handler(Text(startswith='правила', ignore_case=True), content_types=ContentType.TEXT)#просмотр правил
...
@dp.message_handler(Text(startswith='+правила', ignore_case=True), content_types=ContentType.TEXT)#изменение правил чата
...
@dp.message_handler(Text(startswith="кто я", ignore_case=True), content_types=ContentType.TEXT)
async def all_about_self_user(message: types.Message):
    ...
    try:
        ...
        await message.reply(f'Полное описание <a href="tg://user?id={message.reply_to_message.from_user.id}">Пользователя</a> не заполнено', parse_mode="html")
    ...
    try:
        await bot.send_photo(chat_id=message.chat.id, photo=dict((profile_pictures.photos[0][0])).get("file_id"),caption=itog_text, parse_mode=ParseMode.HTML)
    except IndexError:
        await message.reply(itog_text, parse_mode=ParseMode.HTML)

@dp.message_handler(Text(startswith="кто ты", ignore_case=True), content_types=ContentType.TEXT)
async def all_about_user(message: types.Message):
    ...
    itog_text = f'🎅Это пользователь <a href="https://t.me/{username}">{clan_nik_user}</a>\n{chat_status}\n\n{rang}\n\n{about_user}\n\n📨Клановый ник: {clan_nik_user}\n\n{warns}\n\n{recom}'
    try:
        await bot.send_photo(chat_id=message.chat.id, photo=dict((profile_pictures.photos[0][0])).get("file_id"),caption=itog_text, parse_mode=ParseMode.HTML)
    except IndexError:
        await message.reply(itog_text, parse_mode=ParseMode.HTML)
...
@dp.message_handler(Text(startswith='+приветствие', ignore_case=True), content_types=ContentType.TEXT)
...
@dp.message_handler(Text(startswith='приветствие', ignore_case=True), content_types=ContentType.TEXT)#просмотр приветсвия
async def privetstvie(message):
    ...
    cursor = connection.cursor()
    await message.reply(
        f"🗓<b>Приветствие новых пользователей</b>\n\n{cursor.execute(f'SELECT text FROM privets WHERE chat_id=?', (message.chat.id,)).fetchall()[0][0]}",
        parse_mode='HTML')


@dp.message_handler(Text(startswith='!Настройка', ignore_case=True), content_types=ContentType.TEXT)
async def set_new_chat(message):
    ...
    connection.commit()
    table_creation_query = f"""
                        CREATE TABLE [{-(message.chat.id)}muts] (
                            user_id     UNIQUE,
                            rang_moder,
                            moder_id,
                            moder_men,
...
@dp.message_handler(Text(startswith='!изменение чатов', ignore_case=True), content_types=ContentType.TEXT)
async def set_new_chat(message):
    ...
    try:
        ...
        try:
            ...
            await message.reply('📝Неверное использование команды \n\n💬Правильное использование этой команды:\n\n<code>!изменение чатов\nклан:\nсостав 1:\nсостав 2:\nлоги:</code>', parse_mode='HTML')
            return
    try:
...
@dp.message_handler(Text(startswith=['+рекомендация', 'рекомендовать'], ignore_case=True), content_types=ContentType.TEXT)
async def add_recom(message):
    ...
    for user_ids in users_idss:
        ...
        if user_ids[0] == user_id:
            ...
            return
    try:
        comments = text.split('Причина:')[1].split()[0]
    except IndexError:
        try:
            comments = text.split('причина:')[1].split()[0]
        except IndexError:
            ...
            return
    try:
        recom = text.split('Рекомендую на:')[1].split()[0]
    except IndexError:
        try:
            recom = text.split('Рекомендую на:')[1].split()[0]
        except IndexError:
...
async def successful_recom1(call: types.CallbackQuery):
    ...
    pwo = PasswordGenerator()
    id_recom = pwo.shuffle_password('ASDFGHJKL12345678', 8)
    cursor.execute('INSERT INTO recommendation (user_id, pubg_id, moder, comments, rang, date, recom_id) VALUES (?, ?, ?, ?, ?, ?, ?)',(user_id, pubg_id, moder, comments, recom, date, id_recom))
...
@dp.message_handler(Text(startswith=['-рекомендация'], ignore_case=True), content_types=ContentType.TEXT)
async def dell_recom(message):
    ...
    if moder in can_snat_recommend_users:
        ...
        await message.reply('📝Тебе не доступна эта функция\n\n💬<i>Снять свою рекомендацию можно в админ боте</i>', parse_mode='HTML')
        return
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                moder_id = cursor.execute(f"SELECT tg_id FROM [{-(klan)}] WHERE username=?", (username,)).fetchall()[0][0]
            except IndexError:
                await message.reply(
    ...
    if is_this_moder == False:
        ...
        return
    recom_id=cursor.execute('SELECT recom_id FROM recommendation WHERE user_id = ? AND moder = ?', (user_id, moder_id,)).fetchall()[0][0]
    print(recom_id)
...
async def id_user_check(message: types.Message):
    ...
    await message.answer(f'👤 Пользователь <a href="https://t.me/{username}">{name_user}</a>\n🆔 равен @<code>{tg_id}</code>', parse_mode='html', disable_web_page_preview=True)
...
@dp.message_handler(Text(startswith='!список команд', ignore_case=True))
async def id_user_check(message: types.Message):
    connection = sqlite3.connect('Base_bot.db', check_same_thread=False)
    cursor = connection.cursor()
    ...
    await message.answer('🗓Список команд отправлен в <a href="https://t.me/for_klan_tests_bot">лс</a>', parse_mode=ParseMode.HTML, disable_web_page_preview=True)
